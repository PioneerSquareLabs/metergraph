...
`rules` options:
- `input_includes_cache_read: true` — provider reports cached tokens inside `input_tokens`;
  cache reads are deducted from billable input. Supplied on load for the channels whose
  providers do this (`openai-api`, `google-api`, `google-vertex-ai`, `deepseek-api`,
  `xai-api`), so a row only states it to say `false` or to claim it on another channel,
  such as a gateway serving one of those providers.
